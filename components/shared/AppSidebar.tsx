...
export function AppSidebar() {
  return (
    <aside className="flex min-h-screen w-full flex-col border-r border-app-border bg-white lg:w-72">
      ...
      <nav className="flex-1 space-y-1 p-3">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-app-text/90 hover:bg-primary/5 hover:text-primary"
          >
      ...
      <div className="border-t border-app-border p-4">
        <div className="mb-3 flex items-center gap-3 rounded-lg bg-app-background p-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-sm font-semibold text-primary">
            AD
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-app-text">{currentProfile.fullName}</p>
            <p className="text-xs text-app-muted">{roleLabels[currentProfile.role]}</p>
          </div>
          <Bell className="h-4 w-4 text-app-muted/70" />
        </div>
        <form action={logoutAction}>
          <Button variant="ghost" className="w-full justify-start text-app-text/80">
